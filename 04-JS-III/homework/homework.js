...
function numeroMasGrande(numeros) {
  ...
  // Tu código:
  // Version mas corta: Math.max.apply(numeros)
  var cajon=0
  for (var i =0;i< numeros.length; i++){
    if(numeros[i] > cajon){
      cajon= numeros[i];
...
for(var i=0;i<arreglo.length; i++){
  if(arreglo[i] > 18){
    contenedor18s.push(arreglo[i])
  } 
} return contenedor18s.length
}
...
function mesesDelAño(array) {
  ...
  for(var i=0;i<array.length; i++){
    if(array[i]== "Enero" || array[i]== "Marzo"|| array[i]== "Noviembre"){
      arraynewmes.push(array[i])
    }
  }
  if(arraynewmes.length=== 3)
  return arraynewmes
  else 
